...
export default function HomePage() {
    ...
    supabase.auth.getUser().then(({ data }) => {
      ...
      if (data.user) {
        fetch('/api/profile').then(r => r.json()).then(p => {
          setDisplayName(p.display_name || data.user?.user_metadata?.full_name || data.user?.email?.split('@')[0] || '')
        })
      }
    })
  ...
  async function handleNameChange(e: React.FormEvent) {
    ...
    try {
      ...
      fetchPosts(searchQuery) // 목록 새로고침
    } catch (err) {
      setNameError(err instanceof Error ? err.message : '오류가 발생했습니다.')
    } finally {
              ...
              <>
                ...
                <div ref={dropdownRef} className="relative">
                  <button
                    ...
                    className="flex items-center gap-1.5 text-sm text-gray-700 font-medium hover:text-gray-900 border border-gray-200 px-3 py-1.5 rounded-lg hover:bg-gray-50 transition-colors max-w-[120px]"
                  >
                    <span className="truncate">{displayName}</span>
                    <svg className={`w-3.5 h-3.5 text-gray-400 shrink-0 transition-transform ${dropdownOpen ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path d="m6 9 6 6 6-6"/></svg>
                  </button>
                  {dropdownOpen && (
                    <div className="absolute right-0 top-full mt-1.5 w-44 bg-white border border-gray-200 rounded-xl shadow-lg py-1 z-50">
                      <button
                        onClick={() => { setDropdownOpen(false); setNameInput(displayName); setNameError(''); setNameModalOpen(true) }}
                        className="w-full text-left px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 flex items-center gap-2"
                      >
                        <svg className="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                        {lang === 'ko' ? '작성자명 변경' : '投稿者名の変更'}
                      </button>
                      <a
                        href="/my"
                        className="w-full text-left px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 flex items-center gap-2"
                      ...
                      >
                        <svg className="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>
                        {lang === 'ko' ? '내 글 보기' : 'マイ投稿'}
                      </a>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 px-4">
          <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-sm">
            ...
            <p className="text-xs text-gray-500 mb-4">
              {lang === 'ko'
                ? '변경하면 내가 작성한 모든 글·댓글의 이름이 함께 바뀝니다.'
                : '変更すると、投稿したすべての記事・コメントの名前が更新されます。'}
            </p>
